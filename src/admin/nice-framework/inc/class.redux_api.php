    ...
    if ( ! class_exists( 'KevinKissi' ) ) {
        ...
        class KevinKissi {
            ...
            public static function load() {
                add_action( 'after_setup_theme', array( 'KevinKissi', 'createRedux' ) );
                add_action( 'init', array( 'KevinKissi', 'createRedux' ) );
                add_action( 'switch_theme', array( 'KevinKissi', 'createRedux' ) );
            }

            public static function init( $opt_name = "" ) {
                if ( ! empty( $opt_name ) ) {
                    self::loadRedux( $opt_name );
                    remove_action( 'setup_theme', array( 'KevinKissi', 'createRedux' ) );
                }
            }
            ...
            public static function extensionPath( $extension, $folder = true ) {
                if ( ! isset( KevinKissi::$extensions[ $extension ] ) ) {
                    return;
                }
                $path = end( KevinKissi::$extensions[ $extension ] );
                if ( ! $folder ) {
                    return $path;
                }
            ...
            public static function loadRedux( $opt_name = "" ) {
                ...
                if ( isset( self::$uses_extensions[ $opt_name ] ) && ! empty( self::$uses_extensions[ $opt_name ] ) ) {
                    add_action( "redux/extensions/{$opt_name}/before", array( 'KevinKissi', 'loadExtensions' ), 0 );
                }
            ...
            public static function setSections( $opt_name = '', $sections = array() ) {
                ...
                if ( ! empty( $sections ) ) {
                    foreach ( $sections as $section ) {
                        KevinKissi::setSection( $opt_name, $section );
                    }
                }
            }
            ...
            public static function getAllExtensions() {
                ...
                foreach ( $redux as $instance ) {
                    ...
                    if ( ! empty( $instance['extensions'] ) ) {
                        ...
                        KevinKissi::getInstanceExtensions( $instance['args']['opt_name'], $instance );
                    }
                }
            }
            ...
            public static function getInstanceExtensions( $opt_name, $instance = array() ) {
                ...
                foreach ( $instance->extensions as $name => $extension ) {
                    ...
                    if ( isset( $extension->extension_dir ) ) {
                        KevinKissi::setExtensions( $opt_name, str_replace( $name, '', $extension->extension_dir ) );

                    } else if ( isset( $extension->_extension_dir ) ) {
                        KevinKissi::setExtensions( $opt_name, str_replace( $name, '', $extension->_extension_dir ) );
                    }
                }
            }
            ...
            public static function getExtensions( $opt_name = "", $key = "" ) {
                ...
                if ( empty( $opt_name ) ) {
                    ...
                    if ( empty( self::$uses_extensions[ $opt_name ] ) ) {
                        KevinKissi::getInstanceExtensions( $opt_name );
                    }
